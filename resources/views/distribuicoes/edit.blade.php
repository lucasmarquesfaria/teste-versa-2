        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('distribuicoes.update', $distribuicao) }}" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <select id="instituicao_id" name="instituicao_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                ...
                                @foreach($instituicoes as $instituicao)
                                    <option value="{{ $instituicao->id }}" {{ old('instituicao_id', $distribuicao->instituicao_id) == $instituicao->id ? 'selected' : '' }}>
                                        {{ $instituicao->nome }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select id="tipo_certidao" name="tipo_certidao" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>
                                <option value="obito" {{ old('tipo_certidao', $distribuicao->tipo_certidao) == 'obito' ? 'selected' : '' }}>Declaração de Óbito (DO)</option>
                                <option value="nascidos_vivos" {{ old('tipo_certidao', $distribuicao->tipo_certidao) == 'nascidos_vivos' ? 'selected' : '' }}>Declaração de Nascidos Vivos (DNV)</option>
                            </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <x-text-input id="numero_inicial" name="numero_inicial" type="number" class="mt-1 block w-full" value="{{ old('numero_inicial', $distribuicao->numero_inicial) }}" min="1" required />
                                <x-input-error :messages="$errors->get('numero_inicial')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="numero_final" :value="__('Número Final')" />
                                <x-text-input id="numero_final" name="numero_final" type="number" class="mt-1 block w-full" value="{{ old('numero_final', $distribuicao->numero_final) }}" min="1" required />
                        ...
                        <div>
                            ...
                            <x-text-input id="data_entrega" name="data_entrega" type="date" class="mt-1 block w-full" value="{{ old('data_entrega', $distribuicao->data_entrega->format('Y-m-d')) }}" required />
                            <x-input-error :messages="$errors->get('data_entrega')" class="mt-2" />
                        </div>

                        <div>
                            <x-input-label for="observacao" :value="__('Observações')" />
                            <textarea id="observacao" name="observacao" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" rows="3">{{ old('observacao', $distribuicao->observacao) }}</textarea>
