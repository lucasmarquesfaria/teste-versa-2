<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('baixas.store-lote') }}" class="space-y-6">
                        @csrf
                        ...
                        <div>
                            <x-input-label for="numeros" :value="__('Números para Baixa')" />
                            <textarea id="numeros" name="numeros" rows="5" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required>{{ old('numeros') }}</textarea>
                            <x-input-error :messages="$errors->get('numeros')" class="mt-2" />
                            <p class="text-sm text-gray-500 mt-1">
                                Insira os números separados por vírgula (1,2,3) ou especifique intervalos usando hífen (1-5).
                                Exemplo: 10, 15-20, 25, 30-35
                            </p>
                        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Carrega distribuições ao selecionar instituição
            ...
            function atualizarRangeInfo() {
                ...
                if (selectedOption && selectedOption.value) {
                    ...
                    if (numeroInicial && numeroFinal) {
                        ...
                        // Sugerir intervalo completo no campo de números se estiver vazio
                        const numerosTextarea = document.getElementById('numeros');
                        if (!numerosTextarea.value.trim()) {
                            numerosTextarea.value = `${numeroInicial}-${numeroFinal}`;
                        }
                    } else {
